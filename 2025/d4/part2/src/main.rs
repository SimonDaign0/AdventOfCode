...
fn main() {
    let fc = read_input();
    let mut grid: Vec<Vec<char>> = fc.lines().map(|l| l.chars().collect()).collect();
    let mut total_accessed = 0;
    loop {
        // if any paper has been accessed
        let mut has_been_accessed = false;
        for row in 0..grid.len() {
            for col in 0..grid[row].len() {
                ...
                // brought into loop so could borrow grid
        ...
        if !has_been_accessed {
            break;
        }
    }
    println!("amt accessed, {}", total_accessed);
}
